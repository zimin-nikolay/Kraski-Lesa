...
После загрузки репозитория сайт можно опубликовать бесплатно:

1. Открыть **Settings → Pages**.
2. В разделе **Build and deployment** выбрать **Deploy from a branch**.
3. Выбрать ветку `main` и каталог `/ (root)`.
4. Нажать **Save**.

После публикации сайт будет доступен по адресу:

```text
https://zimin-nikolay.github.io/Kraski-Lesa/
```

## Статус проекта

Завершённый учебный веб-проект, подготовленный для демонстрации навыков адаптивной верстки и разработки интерактивных интерфейсов на JavaScript.
